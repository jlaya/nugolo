<?php
    // $student_list = $this->crud_model->all_enrolled_student()->result_array();
    $student_list = $this->user_model->get_user()->result_array();
    // Lista de rutinas
    $rutina_list = $this->user_model->get_rutinas()->result_array();

?>
<div class="mail-header" style="padding-bottom: 27px ;">
    <!-- title -->
    <h4 class="mail-title">
        <?php echo get_phrase('write_new_message'); ?>
    </h4>
</div>

<div class="mail-compose">

    <?php echo form_open(site_url('admin/message/send_new'), array(
            'class' => 'form-groups form-horizontal', 'enctype' => 'multipart/form-data')); ?>


    <div class="form-group">
        <label for="subject">Rutina:</label>
        <br><br>
        <select class="form-control select2" id="rutina" required>

            <option value="">Seleccionar una rutina</option>
            <?php
            foreach($rutina_list as $row):?>
                <option value="<?php echo $row['id']; ?>">
                    - <?php echo $row['title']; ?></option>
            <?php endforeach; ?>
        </select>
        <br><br>
        <select class="form-control select2" id="reciever" name="reciever" required>
            <option value=""><?php echo get_phrase('select_a_user'); ?></option>
        </select>
    </div>


    <div class="compose-message-editor">
        <textarea rows="5" class="form-control wysihtml5" data-stylesheet-url="<?php echo base_url('assets/backend/css/wysihtml5-color.css');?>"
            name="message" placeholder="<?php echo get_phrase('write_your_message'); ?>"
            id="sample_wysiwyg" required></textarea>
    </div>

    <hr>

    <button type="submit" class="btn btn-success pull-right">
        <i class="fa fa-share"></i> &nbsp;<?php echo get_phrase('send_message'); ?>
    </button>
</form>

</div>

<script type="text/javascript">
    $('#rutina').change(function(){
        
        var course_id = $(this).val();
        $.ajax({
           url:'<?php echo base_url("Admin/ajax_users_rutinas"); ?>',
           dataType: "json",
           type:'post',
           data:{ 'course_id' : course_id },
           success:  function (r) {
                var option = "";
                $.each(r, function(index, value){
                    option += "<option value='"+value.id+"'>"+value.first_name+" "+value.last_name+"</option>";
                });
                //$('#reciever option[value=""]').remove();
                //$("#reciever").append(option);
                $('#reciever')
                .empty()
                .append(option);
            
           },
           statusCode: {
              404: function() {
                 alert('Not found');
              }
           },
           error:function(x,xs,xt){
              //window.open(JSON.stringify(x));
              //alert('error: ' + JSON.stringify(x) +"\n error string: "+ xs + "\n error throwed: " + xt);
           }
        });

    });
</script>
