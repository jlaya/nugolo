...
class Admin extends CI_Controller {
    ...
    public function users($param1 = "", $param2 = "",$param3=2) {
        ...
        if ($param1 == "add") {
            ...
            $this->user_model->add_user($param3);
            redirect(site_url('admin/users'.$refresh), 'refresh');
        }
        elseif ($param1 == "edit") {
        ...
        $page_data['users'] = $this->user_model->get_user($param2,$rol);
        //print_r($page_data['users']->result_array());echo $param2; die;
        $this->load->view('backend/index', $page_data);
    }

    public function user_form($param1 = "", $param2 = "",$param3="") {
        switch ($param3) {
            case 1:
                $name='admin';
                $para='/0/0/1';
                $page_data['rol']='/0/1';
                $page_data['rol_edit']='/1';
                break;
            case 3:
                $name='instructor';
                $para='/0/0/3';
                $page_data['rol']='/0/3';
                $page_data['rol_edit']='/3';
                break;
            default:
                $name='students';
                $para='';
                $page_data['rol']='/0/2';
                $page_data['rol_edit']='/2';
                break;
        }
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        ...
        if ($param1 == 'add_user_form') {
            ...
            $page_data['page_title'] = get_phrase($name.'_add');
        ...
        elseif ($param1 == 'edit_user_form') {
            ...
            $page_data['page_title'] = get_phrase($name.'_edit');
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function enroll_student($param1 = "") {
        ...
        $page_data['page_name']   = 'enroll_student-new';
        $page_data['page_title']  = get_phrase('enroll_a_student');
        $page_data['user_detail'] = $this->user_model->detail_user_detail();
        $this->load->view('backend/index', $page_data);
    }

    // Asociacion Curso con Profesor
    public function enroll_teacher_course($param1 = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == 'enroll') {
            $this->crud_model->enroll_a_teacher_course_manually();
            redirect(site_url('admin/enroll_teacher_course'), 'refresh');
        }
        
        $this->session->set_userdata('last_page', 'enroll_student');
        $page_data['page_name']   = 'enroll_teacher_course';
        $page_data['page_title']  = "Relacion Profesor/Cursos";
        $page_data['user_detail'] = $this->user_model->enroll_teacher_course();
    ...
    public function is_checked(){
        ...
        return $this->db->update('users_detail',  array( 'is_checked' => $data['is_checked'] ) );
    }
    ...
    // Activacion de usuarios clientes
    ...
    public function enroll_history_delete($id = "", $id_refresh = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        $this->crud_model->delete_enroll_history($id);
        $this->session->set_flashdata('flash_message', get_phrase('data_deleted_successfully'));

        if( $id_refresh == "" ){
            redirect(site_url('admin/enroll_history'), 'refresh');
        }else{
            redirect(site_url('Admin/show_enroll/'.$id_refresh), 'refresh');
        }
    ...
    // Lista de estudiantes asociados a el curso
    public function show_enroll( $id ) {
        $page_data['course'] = $this->crud_model->row_course( $id );
        $page_data['page_title'] = 'Matriculados';
        $page_data['logged_in_user_role'] = strtolower($this->session->userdata('role'));
        $page_data['obj'] = $this->crud_model->show_enroll( $id );
        $this->load->view("backend/".$page_data['logged_in_user_role']."/list-enroll", $page_data );
    }
    ...
    public function courses($default_category_id = "", $default_sub_category_id = "", $default_instructor_id = "") {
        ...
        if (isset($_POST['category_id']) && isset($_POST['sub_category_id'])) {
            ...
            if($default_category_id == "" && $default_sub_category_id == ""){
                $page_data['default_category_id'] = '';
                $page_data['default_sub_category_id'] = '';
            }else {
    ...
    // AJAX PORTION
    public function ajax_users_rutinas() {
        $course_id = $this->input->post('course_id');
        $obj = $this->user_model->show_users_rutinas($course_id)->result_array();
    ...
    public function logs_pf(){
       
        $page_data['logspf']   = $this->crud_model->get_transactionPF();
        
        $this->session->set_userdata('last_page', 'logs_pf');
        $page_data['page_name']  = 'logs_pf';
        $page_data['page_title'] = get_phrase('logs').' '.get_phrase('paguelo_facil');
         $this->load->view('backend/index',$page_data);
    }
    ...
    public function verify_email(){
        ...
        $result = $this->db->get('users AS a')->num_rows();
        echo $result;
    }


    public function filter_logros( $user_id )
    {   
        $this->db->select('b.*');
    ...
    // Reporte para mostrar historial de logros segun el estudiante
    public function pdf( $user_id ){

        $obj = $this->filter_logros( $user_id );

        #print_r($obj);
        #exit;

        $this->load->view('frontend/pdf/logros', compact('obj'));
        ...
        $this->dompdf->stream("Logros (Estudiante $user_id).pdf", array("Attachment"=> 0));
